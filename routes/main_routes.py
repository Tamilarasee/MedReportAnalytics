...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            # Process the PDF
            extracted_text = extract_text_from_pdf(file, use_llm=True)

            if not extracted_text:
                flash(
                    'Could not extract text from the PDF. Please try another file.', 'danger')
            ...
            enriched_data = enrich_data(
                processed_data, conditions, use_llm=True)
...
def query_report():
    ...
    try:
        ...
        answer = process_query(
            query, report['processed_data'], report['conditions'], report['enriched_data'])
